...
impl PacketMotionData {
    fn from_2022(header: PacketHeader, motion_data: RawMotionData) -> Self {
        let car_motion = motion_data
            .car_motion
            .iter()
            .map(CarMotionData::from_2022)
            .collect();
...
impl CarMotionData {
    fn from_2022(car_motion: &RawCarMotion) -> Self {
        Self {
...
pub(crate) fn parse_motion_data<T: BufRead>(
    ...
    Ok(PacketMotionData::from_2022(header, motion_data))
}
