...
pub(crate) fn parse_packet(size: usize, packet: &[u8]) -> Result<Packet, UnpackError> {
    ...
    match packet_id {
        PacketType::Motion => {
            let packet = parse_motion_data(&mut cursor, header)?;
        ...
        PacketType::Participants => {
            let packet = parse_participants_data(&mut cursor, header)?;
        ...
        PacketType::CarTelemetry => {
            let packet = parse_car_telemetry_data(&mut cursor, header)?;
